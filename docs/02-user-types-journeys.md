...
## 2.3 Journey map — Normal traveler (flight, MVP semi-manual flow)

| Stage | User action | System/staff action | Emotion target |
|---|---|---|---|
| Discover | Opens app, picks Arabic | Localized home, popular routes from TIP/BEN | Welcome, familiar |
| Search | Tripoli → Istanbul, dates, 2 pax | Shows results (cached fares/on-request in MVP) | Confidence: clear prices |
| Compare | Filters: direct, baggage | Labels: cheapest / best value / fastest / family-friendly | Control |
| Details | Enters passengers or picks saved travelers | Passport expiry check → warning if <6 months validity | Cared for |
| Pay | Chooses bank transfer, uploads receipt | Status: *Pending payment* → finance confirms → *Payment received* | Trust |
| Fulfill | Waits | Staff issues ticket, uploads PDF; status → *Ticket issued* | Relief |
| Receive | Gets app notification + WhatsApp PDF + email | Automated dispatch of ticket + trip added to My Trips | Delight |
| Pre-trip | — | T-24h flight reminder, check-in reminder, baggage note | Supported |
...
5. "Request quotation" creates an admin task; staff verify prices and send an official branded PDF within SLA (e.g., 2 working hours).
6. All AI prices labeled **"سعر تقريبي / Estimated price"** until staff confirm.

## 2.8 Cross-journey rules

- Every journey exposes a persistent **WhatsApp support** entry point.
